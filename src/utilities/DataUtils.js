...
export const getHourlyForecastWeather = (response, descriptions_list) => {
  let foreacast_data = [];
  let descriptions_data = [];
  let grouped_forecast_data = {}
  let grouped_forecast_descriptions = {}

  if (!response)
    return [];
  else
    response?.hourly.slice().map((item, idx) => {
      const dt = new Date(item.dt * 1000)
      const parsedDt = dt.getFullYear() + "-" + (dt.getMonth() + 1) + "-" + dt.getDate()
      let paresdTm = dt.getHours()
      if(paresdTm < 10)
        paresdTm = "0" + paresdTm.toString() + ":00"
      else
      paresdTm = paresdTm.toString() + ":00"
      ...
      grouped_forecast_data[paresdTm] = [{
        date: parsedDt,
        temp: item.temp,
        humidity: item.humidity,
        wind: item.wind_speed,
      ...
      grouped_forecast_descriptions[paresdTm] = [{
        description: item.weather[0].description,
        date: parsedDt,
        time: paresdTm,
      }]
